...
//    AVAssetResourceLoader
    NSString *path = [[NSBundle mainBundle] pathForResource:@"mao_snow" ofType:@"mp4"];
    NSURL *url;
    if (path) {
    ...
    self.slider.continuous = NO;    // 设置 slider.continuous = NO 可使 ValueChanged 事件仅在拖动结束时触发
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"status"]) {
        if ([playerItem status] == AVPlayerItemStatusReadyToPlay) {
            ...
            // 转换成时间字符串
            self.totalTimeStr = [self convertTime:totalSecond];
...
- (void)monitoringPlayback:(AVPlayerItem *)playerItem {
    ...
    _playbackTimeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:NULL usingBlock:^(CMTime time) {
        ...
        weakSelf.slider.value = currentSecond;
        weakSelf.timeLabel.text = [NSString stringWithFormat:@"%@\n%@", [weakSelf convertTime:currentSecond], weakSelf.totalTimeStr];
    }];
}
