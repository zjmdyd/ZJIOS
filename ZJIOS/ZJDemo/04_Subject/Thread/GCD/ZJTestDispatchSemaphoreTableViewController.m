...
- (void)initAry {
    ...
    self.cellTitles = @[@"test0", @"test1_no_semahpore", @"test2_semahpore"];
}
...
- (void)test0 {
    ...
    dispatch_async(queue, ^{
        ...
        dispatch_semaphore_signal(semahpore);   // 如果不发送信号,dispatch_semaphore_wait则会一直等待直至超时
    });
    NSLog(@"before_wait:%@", [NSThread currentThread]); // 打印2, 打印2之后信号量变为-1进入阻塞状态
    // semaphore -1, < 0，当前线程阻塞，进入等待状态
    dispatch_semaphore_wait(semahpore, DISPATCH_TIME_FOREVER);
...
// data race数据竞争, 添加synchronized可解决
- (void)saleTickNoSafe {
    while (1) {
        @synchronized (self) {
            if (_count > 0) {   // 有票，继续卖
...
/*
 ...
 并发任务，窗口A继续卖票，执行操作1，收紧资源, 此时分两种情况:
 1.窗口B如果未卖完票，则窗口A需要等待窗口B卖完票，释放资源后才能卖票，
 2.如果窗口B已经卖完票，则窗口A可正常卖票
 重复此上步骤，
 */
- (void)saleTickSafe {
